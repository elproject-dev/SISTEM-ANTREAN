...
export function formatReceipt(transaction: any): string {
  ...
  const DECO_SEPARATOR = '='.repeat(32);
  ...
  receipt += formatLine('No.ID', id ? formatInvoiceNumber(id) : '-') + '\n';
  receipt += formatLine('Waktu', waktuStr) + '\n';

  if (customerType === 'member' && customerName) {
    receipt += formatLine('Pelanggan', `Member - ${customerName}`) + '\n';
  } else {
    receipt += formatLine('Pelanggan', 'Umum') + '\n';
  }
  receipt += formatLine('Kasir', displayCashierName) + '\n';
  receipt += `${SEPARATOR}\n`;

  // Daftar item (nama produk di baris pertama, qty x harga di baris kedua)
  items?.forEach((item: any) => {
    ...
    receipt += formatLine(`${qty}x${formatPrice(price)}`, formatPrice(itemTotal)) + '\n';
  });
  ...
  // Subtotal dan Total
  receipt += formatLine('Subtotal', formatPrice(subtotal)) + '\n';

  // PPN (Pajak 11%)
  if (tax > 0) {
    receipt += formatLine(`Pajak (${ppnPercentage || 11}%)`, formatPrice(tax)) + '\n';
  }

  // Diskon dan poin yang digunakan (di bawah subtotal)
  if (discount > 0) {
    receipt += formatLine('Diskon', formatPrice(discount)) + '\n';
  }
  ...
  receipt += formatLine('TOTAL', formatPrice(total)) + '\n';
...
export function generateReceiptRaw(data: ReceiptData): string {
  ...
  receipt += formatLine('Pelanggan', customerName) + '\n';
  receipt += formatLine('Kasir', displayCashierName) + '\n';
  receipt += `${SEPARATOR}\n`;
  // ==================== ITEMS ====================
  items.forEach((item) => {
    ...
    receipt += truncatedName + '\n';
    receipt += formatLine(`${quantity}x${formatPrice(price)}`, formatPrice(itemTotal)) + '\n';
  });
  ...
  // PPN (Pajak)
  if (tax > 0) {
    receipt += formatLine(`Pajak (${ppnPercentage}%)`, formatPrice(tax)) + '\n';
  }
